...
class InsideTradeController extends ApiController
{
    ...
    public function storeEntrust(Request $request)
    {
        ...
        $params_data = $request->all();
        if(Arr::get($params_data,'type')==2 &&
            Arr::get($params_data,'amount')==0 &&
            Arr::get($params_data,'total')
        ){
            $params_data['amount'] = Arr::get($params_data,'total');
        }
        if ($vr = $this->verifyField($params_data,[
        ...
        $user = $this->current_user();
        $params = collect($params_data)->only(['type','symbol','entrust_price','amount','trigger_price','total'])->toArray();
