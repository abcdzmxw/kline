...
class ContractController extends ApiController
{
    ...
    public function getMarketInfo(Request $request)
    {
        ...
        if(strpos($symbol,'/') == false){
            $symbol_bi = strtolower($symbol).'usdt';
        }
        ...
        // 获取bibi行情
        // $buyList = Cache::store('redis')->get('market:' . $symbol_bi . '_depth_buy') ?: [];
        // $sellList = Cache::store('redis')->get('market:' . $symbol_bi . '_depth_sell') ?: [];
        // $tradeList = Cache::store('redis')->get('market:' . $symbol_bi . '_newPriceBook') ?: [];
        ...
        foreach ($coins as $coin => $class){
            if($symbol == $coin){
                ...
                if(blank($kline)){
                    ...
                    /*if(empty($kline_cache_data['close'])){
