...
class ContractAnomalyController extends AdminController
{
    ...
    public function statistics()
    {
        ...
        $con = '<code>总转入：'.(real)abs($total_in).'USDT</code> ' . '<code>总转出：'.(real)abs($total_out).'USDT</code> ' . '<code>总手续费：'.(real)abs($total_fee).'USDT</code> ' . '<code>总资金费：'.(real)abs($total_cost).'USDT</code> ' . '<code>总盈亏：'.(real)$total_profit.'USDT</code> ' . '<code>总异常资金：'.(real)$total_anomaly.'USDT</code> ' . '<code>总实际余额：'.(real)$total_usable_balance.'USDT</code> ' . '<code>总理论余额：'.(real)$total_theory_balance.'USDT</code> ';
