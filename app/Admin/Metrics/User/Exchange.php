...
class Exchange extends Bar
{
    ...
    public function handle(Request $request)
    {
        ...
        $builder1 = InsideTradeOrder::query()->where(function($q)use($referrer){
                        $q->whereHas('buy_user',function ($q)use($referrer){$q->where('user_id',$referrer);})
                            ->orWhereHas('sell_user',function ($q)use($referrer){$q->where('user_id',$referrer);});
                    })->where('base_coin_id',$coin_id);
        $builder2 = InsideTradeOrder::query()->where(function($q)use($referrer){
                        $q->whereHas('buy_user',function ($q)use($referrer){$q->where('user_id',$referrer);})
                            ->orWhereHas('sell_user',function ($q)use($referrer){$q->where('user_id',$referrer);});
